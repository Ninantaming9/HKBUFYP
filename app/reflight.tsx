...
const FlightDetailScreen = () => {
  ...
  const [bookHistory, setBookHistory] = useState<any>(null);
  const { flightId } = route.params as { flightId: string };
  const { flightBookId } = route.params as { flightBookId: string }; 
  ...
  console.log("bbbbbbbbb"+flightBookId)
  ...
  const fetchPaymentSheetParams = async () => {
    ...
    const response = await fetch(`${API_URL}/paymentsheet`, {
      ...
      body: JSON.stringify({ amount: bookHistory.totalPrice }),
    });
  ...
  useEffect(() => {
    const fetchFlightDetails = async () => {
      try {
            ...
            const response = await fetch(`${API_URL}/findReFlightBookId`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ flightBookId }), 
            });

            if (response.ok) {
                const data = await response.json();
                setBookHistory(data); 
                console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" +  bookHistory?.totalPrice); 
  ...
  const handleSubmit = async () => {
    try {
      console.log('Flight ID:', flightBookId); // 在调用前打印 flightId
      console.log('Flight ID:', flightId); // 在调用前打印 flightId
      const response = await fetch(`${API_URL}/updateFlightBook`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ flightId: flightBookId }), // 确保使用正确的变量
    });
    const data = await response.json();
    ...
    if (response.ok) {
        ...
        setResponseMessage(data.message || 'error,please。');
    }
            ...
            <View style={styles.container}>
              ...
              {errorMessage ? (
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', marginTop: 15 }}>
                  ...
                  <Text style={{ fontSize: 15, fontWeight: 'bold', color: 'black' }}>  Total Price: ${parseFloat(bookHistory?.totalPrice !== undefined ? String(bookHistory.totalPrice) : '').toFixed(2)}</Text>
                </View>
